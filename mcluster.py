...
class Cluster(clust):
    ...
    def updateClusterProperties(self, keys, ntwk, pc_min, pc_max, shrinkMode):
        return updateClusterProperties_numba(keys, self.sizes, self.volume, 
            self.volShrink, self.volMax, self.pc, self.pcShrink, self.pcMax,
            self.hasFluid, self.toImbibe, self.toDrain, self.members, 
            self.mem_offsets, self.valClust, self.valClustD, ntwk.totElements, 
            ntwk.connectivity_graph_flat, ntwk.cg_offsets, ntwk.PcI, ntwk.PcD, 
            ntwk.entryPress_freezed, pc_min, pc_max, shrinkMode)


    def removeMembers(self, keys, toImbibe, ntwk):
        return removeMembers_numba(keys, toImbibe, self.sizes, self.moles, self.hasFluid, 
            self.clusterID, self.pc, self.members, self.mem_offsets, self.heads_arr,
            ...
            ntwk.gasConc, ntwk.volarray, ntwk.minConc, ntwk.connectivity_graph_flat, ntwk.cg_offsets, 
            ntwk.toInlet, ntwk.toOutlet, ntwk.toInBdr, ntwk.toOutBdr, ntwk.elemToUpdateW, 
            ntwk.elemToUpdateNW,_temp.tmepID, _temp.done, _temp.visited)
    ...
    def addMembers(self, keys, toAdd, ntwk):
        ...
        keys_to_update, new_keys = addMembers_numba(updated_keys, updated_toAdd,
            self.pc[updated_keys], self.sizes, self.moles, self.hasFluid, self.clusterID,
            self.pc,  self.members, self.mem_offsets, self.heads_arr, self.next_elem_arr, 
            self.trapped, self.trappedStatus, self.conn, self.connected, self.valClust, 
            ntwk.aqueousMoles, ntwk.connectivity_graph_flat, ntwk.cg_offsets, 
            ntwk.toInlet, ntwk.toOutlet, ntwk.toInBdr, ntwk.toOutBdr, _temp.tempID, 
            _temp.done, _temp.visited, self.neighbours_updated)
    ...
    def returnClusterPcVolume(self, keys, ntwk, is_oil_inj, moles_tol, pc_tol):
        #print("Im in returnClusterPcVolume")
        ...
        returnClusterPcVolume_numba(keys, ntwk.m_halfAngles, ntwk.m_cornExists,
            ...
            ntwk.imposedP, ntwk.MOLECULAR_LENGTH, ntwk._delta, 
            ntwk.H, ntwk.RT, 1e-3, ntwk.maxPc, moles_tol, pc_tol)
    ...
    def adjustClusterPcVolume(self, ntwk, moles_tol, pc_tol):
        adjustClusterPcVolume_numba(self.netMolesAfterLastUpdate, ntwk.m_halfAngles, 
            ntwk.m_cornExists, ntwk.m_inited, ntwk.m_initOrMaxPcHist, 
            ntwk.m_initOrMinApexDistHist, ntwk.m_advPc, ntwk.m_recPc, ntwk.m_initedApexDist, 
            ntwk.totElements, ntwk.sigma, ntwk.thetaAdvAng, ntwk.thetaRecAng, 
            ntwk.nCorners_arr, ntwk.areaSPhase, ntwk._cornArea, ntwk._centerArea, 
            ...
            self.members, self.mem_offsets, self.sizes, _temp.tempCID, ntwk.gasConc,
            ntwk.imposedP, ntwk.MOLECULAR_LENGTH, ntwk._delta, ntwk.H, ntwk.RT, 1e-3, ntwk.maxPc, moles_tol, pc_tol)
    ...
    def shrinkCluster(self, keys, ntwk, moles_tol=1e-20, pc_tol=1e-4, 
        ...
        for k in keys:
            ntwk.entryPress_freezed[self[k].members] = False
        ...
        parallel_exploration_dsu_with_assigning(toImbibe, self.pc[keys], cWP.hasFluid, 
            ntwk.connectivity_graph_flat, ntwk.cg_offsets, cWP.sizes, cWP.clusterID, 
            cWP.pc, _temp.tempID, ntwk.toInlet, ntwk.toOutlet, ntwk.toInBdr, 
            ntwk.toOutBdr, _temp.done, _temp.visited, cWP.members, cWP.mem_offsets,
            cWP.heads_arr, cWP.next_elem_arr, cWP.trapped, cWP.trappedStatus, 
            cWP.conn, cWP.connected, cWP.neighbours_updated, True)
        ntwk.satList[toImbibe] = 1.0

        # remove phase 1 from toImbibe
        
        _temp.tempID[:] = -5
        keys_to_update = removeMembers_numba(keys, toImbibe, self.sizes, self.moles, 
            ...
            self.conn, self.connected, self.neighbours_updated, ntwk.aqueousMoles, 
            ntwk.minMoles, ntwk.gasConc, ntwk.volarray, ntwk.minConc, ntwk.connectivity_graph_flat, 
            ntwk.cg_offsets, ntwk.toInlet, ntwk.toOutlet, ntwk.toInBdr, ntwk.toOutBdr, 
            ntwk.elemToUpdateW, ntwk.elemToUpdateNW, _temp.tempID, _temp.done, _temp.visited)
    ...
    def growCluster(self, keys, ntwk, moles_tol=1e-23, pc_tol=1e-4, max_iter=50, 
        ...
        ntwk.entryPress_freezed[toDrain] = False
        for k in keys:
            ntwk.entryPress_freezed[self[k].members] = False
        ntwk.fluid[toDrain] = 1 
        
        #from IPython import embed; embed()
        oldVolume = self.volume[keys]
        oldMoles = self.moles[keys]
        oldPc = self.pc[keys]
...
@njit(void(boolean[:], boolean[:], boolean[:], float32[:], float32[:],
    ...
    boolean[:], float64[:], float64[:], float64, float64, float64, float64, float64, int64), 
...
def updateArrays(hasFluid, validToShrink, validToGrow, pcShrink, pcMax, 
    ...
    validVolumes, gasConc, aqueousMoles, initialAvgPres, minConc, imposedP, Pc, H, totElements):
    ...
    for i in prange(totElements):
        ...
        if hasFluid[i]: 
            ...
            #clusterMoles[cid] += satList[i]*volarray[i]*minConc
            aqueousMoles[i] = gasConc[i]*volarray[i]
            #gasConc[i] = H*clusterPc[cid] 
            #if pcShrink[cid] < initialAvgPres:
            #    validToShrink[cid] = False 
            #if pcMax[cid] > initialAvgPres:
            #    validToGrow[cid] = False
        else: 
            elemToUpdateW[i] = True
            #gasConc[i] = H*(initialAvgPres + imposedP)
            gasConc[i] = minConc
            #gasConc[i] = H*initialAvgPres
...
@njit(int32[:](
    int32[:], int32[:], int32[:], float64[:], boolean[:], int32[:], float32[:],
    int32[:], int32[:], int32[:], int32[:], boolean[:], boolean[:], boolean[:], 
    boolean[:], boolean[:], float64[:], float64[:], float64[:], float64[:], float64,
    int32[:], int32[:], boolean[:], boolean[:], boolean[:], boolean[:], boolean[:], boolean[:], 
    int32[:], boolean[:], int32[:]), cache=True)
def removeMembers_numba(keys, toRemove, sizes, moles, hasFluid, clusterID, clusterPc, 
    members, mem_offsets, heads_arr, next_elem_arr, trapped, trappedStatus, conn, 
    connected, neighbours_updated, aqueousMoles, minMoles, gasConc, volarray, minConc,
    cg_data, cg_offsets, toInlet, toOutlet, toInBdr, toOutBdr, etoUpdateW, etoUpdateNW, 
    temp_clusterID, done, visited):
    ...
    for i in range(nkeys):
        ...
        if notAdded[k]:
            ...
            t += 1
        if sizes[k]==1:
            aqueousMoles[toImb] = moles[k]
            gasConc[toImb] = minConc + aqueousMoles[toImb]/volarray[toImb]
            moles[k] = 0.0
        else:
            #aqueousMoles[toImb] = min(moles[k], minMoles[toImb])
            aqueousMoles[toImb] = 0.0
            #moles[k] -= aqueousMoles[toImb]
            gasConc[toImb] = minConc
        ...
        Pc = clusterPc[k]
        parallel_exploration_dsu_with_assigning(neigh,  np.asarray([Pc]), hasFluid, cg_data, 
            cg_offsets, sizes, clusterID, clusterPc, temp_clusterID, toInlet, toOutlet, 
            toInBdr, toOutBdr, done, visited, members, mem_offsets, heads_arr, 
            next_elem_arr, trapped, trappedStatus, conn,  connected, neighbours_updated, 
            True)
...
def addMembers_helper(keys, toAdd, hasFluid, isCircle, isPore, cg_data, cg_offsets, 
    ...
    for i in range(nkeys):
        ...
        hasFluid[toDra] = True
        etoUpdateW[toDra], etoUpdateNW[toDra] = etoUpdateNW[toDra], etoUpdateW[toDra]
        updated_keys[j] = k
...
@njit(Tuple((int32[:], int32[:]))(
    int32[:], int32[:], float32[:], int32[:], float64[:], boolean[:], int32[:], float32[:],
    int32[:], int32[:], int32[:], int32[:], boolean[:], boolean[:], boolean[:], 
    boolean[:], boolean[:], float64[:], int32[:], int32[:], boolean[:], boolean[:], boolean[:], 
    boolean[:], int32[:], boolean[:], int32[:], boolean[:]), cache=True)
def addMembers_numba(keys, toAdd, pc_val, sizes, moles, hasFluid, clusterID, clusterPc,
    members, mem_offsets, heads_arr, next_elem_arr, trapped, trappedStatus, conn, 
    connected, valClust, aqueousMoles, cg_data, cg_offsets, toInlet, toOutlet, toInBdr, 
    toOutBdr, temp_clusterID, done, visited, neighbours_updated):
    ...
    for i in range(nkeys):
        k, toDra = keys[i], toAdd[i]
        moles[k] += aqueousMoles[toDra]
        aqueousMoles[toDra] = 0.0
        #hasFluid[toDra] = True
        if notdone[k]:
            keys_to_update[j] = k
            notdone[k] = False
    ...
    temp_clusterID[:] = -5
    parallel_exploration_dsu_with_assigning(toAdd, pc_val, hasFluid, cg_data, cg_offsets, 
        sizes, clusterID, clusterPc, temp_clusterID, toInlet, toOutlet, toInBdr, toOutBdr,
        done, visited, members, mem_offsets, heads_arr, next_elem_arr, trapped, 
        trappedStatus, conn, connected, neighbours_updated, True)
    ...
    return keys_to_update[:j], new_keys[:n]
...
@njit(void(
    ...
    float32[:], float32[:], float32[:], float32[:], boolean[:], int64, float64, float64), 
    cache=True)
def updateToDrainImbibe_single(k, mem, neigh, toDrainArray, toImbibeArray, 
    pcShrinkArray, pcMaxArray, PcI, PcD, entryPress_freezed, totElements, pc_min, pc_max):
...
@njit(int32[:](
    ...
    float32[:], float32[:], boolean[:], float64, float64, boolean), cache=True)
def updateClusterProperties_numba(keys, sizes, clusterVol, volShrink, volMax,
    ...
    PcI, PcD, entryPress_freezed, pc_min, pc_max, shrinkMode):
    ...
    for i in range(nkeys):
        ...
        if sizes[k]==0:
            ...
            updateToDrainImbibe_single(k, mem, neigh, toDrainArr, toImbibeArr, 
                pcShrink, pcMax, PcI, PcD, entryPress_freezed, totElements, pc_min, pc_max)
...
def checkEvents(sizes, clusterPc, pcShrink, pcMax, validToShrink,
    ...
    for k in range(nClusters):
        ...
        if validToGrow[k] and clusterPc[k] > 1.03*pcMax[k]:
            toGrowList[j] = k
            j += 1
        elif validToShrink[k] and clusterPc[k] < 0.97*pcShrink[k]:
            #if sizes[k]==1 and clusterPc[k] > initialAvgPres:
            #    continue
            toShrinkList[i] = k
            i += 1
...
def compute_fluxes_moles_pc_conc_numba(flux, netFlux, len_tij_valid, gasConc, volarray,
    ...
    aqMoles, minMoles, minConc, imposedP, D, H, RT, _dt, MOLES_EPS):
    ...
    for i in range(fSize):
        ...
        if f > 0.0:
            if (aqMoles[p] - minMoles[p]) <= MOLES_EPS :
                f = 0.0
        elif f < 0.0:
            if (aqMoles[t] - minMoles[t]) <= MOLES_EPS:
                f = 0.0
    ...
    for e in range(nfSize):
        ...
        if elemToUpdateNW[e]:
            ...
            aqMoles[e] += nMoles
            gasConc[e] = minConc + aqMoles[e]/volarray[e]
    ...
    for k in range(nClusters):
        ...
        #disMoles = max(0.0, (volTotal-clust_volume[k])*minConc)
        #pc_k = max(1e-3, (clust_moles[k]-disMoles)*RT/clust_volume[k] - imposedP)
        if clust_volume[k] > 0.0:
            clust_moles[k] = newMole_k
            pc_k = max(1e-3, clust_moles[k]*RT/clust_volume[k] - imposedP)
        else:
            pc_k = max(1e-3, newMole_k/clust_moles[k]*(imposedP+clust_pc[k]) - imposedP)
            clust_moles[k] = newMole_k
...
@njit(Tuple((float64, float64, float64, float64, float64, float64))(
    ...
    float64[:], boolean[:], float32[:], float64[:], float64, int64, boolean[:],
    float64, float64, float64), cache=True)
def statistics_numba(elemToUpdateW, elemtoUpdateNW, aqMoles, satList, volarray, 
    clust_moles, valClust, clusterPc, clusterVol, minConc, totElements, isinsideBox,
    ...
    avgAqPres = total_moles_in_aq/(H*total_aq_vol) + minConc/H - imposedP

    return total_moles_in_gas, total_moles_in_aq, total_moles, satW, avgGasPres, avgAqPres
    
    
@njit(void(int32[:], float64, float64[:], float32[:], float32[:], float64[:]), cache=True, inline='always')
def updateSatConc_numba(mem, new_conc, satList, _cornArea, areaSPhase, gasConc):
...
def returnClusterPcVolume_numba(keys, halfAng, m_exists, m_inited, m_initOrMaxPcHist, 
    ...
    pcShrink, pcMax, clusterMoles, members, mem_offsets, sizes, gasConc, 
    imposedP, molecular_length, _delta, H, RT, minPc, maxPc, moles_tol, pc_tol):

    accurate = False
    nClusters = clusterPc.size
    #temp_pc = np.empty(totElements, dtype=np.float32)
    ...
    for i in range(nkeys):
        ...
        if abs(molesC - moles_k) <= moles_tol:
            ...
            updateSatConc_numba(mem, new_conc, satList, _cornArea, areaSPhase, gasConc)
            continue
        elif moles_k < molesC:
            ...
            pc2 = maxPc
            vol2, moles2 = returnClusterVolMoles(k, mem, pc2, halfAng, m_exists, m_inited, 
                m_initOrMaxPcHist, m_initOrMinApexDistHist, advPc, recPc, m_initedApexDist, 
                is_oil_inj, totElements, sigma, thetaAdvAng, thetaRecAng, nCorners_arr, areaSPhase, 
                _cornArea, _centerArea, conAng_cur, apexDist_cur, volarray, satList, clusterID, 
                imposedP, molecular_length, _delta, RT, False, first_time)
        ...
        #########################################################################################################
        # # at minimum possible pc
        # volM, molesM = returnClusterVolMoles(k, mem, minPc, halfAng, m_exists, m_inited, 
            # m_initOrMaxPcHist, m_initOrMinApexDistHist, advPc, recPc, m_initedApexDist,
            # is_oil_inj, totElements, sigma, thetaAdvAng, thetaRecAng, nCorners_arr, areaSPhase, 
            # _cornArea, _centerArea, conAng_cur, apexDist_cur, volarray, satList, clusterID,
            # imposedP, molecular_length, _delta, RT, accurate, first_time)
        # first_time = False
        
        # if molesM > moles_k or abs(molesM - moles_k) < moles_tol:
            # clusterPc[k] = minPc
            # clusterVol[k] = volM
            # new_conc = H * (imposedP + minPc)
            # #new_conc = H * clusterPc[k]
            # updateSatConc_numba(mem, new_conc, satList, _cornArea, areaSPhase, gasConc)
            ...
            # continue
        # else:
            # vol1, moles1, pc1 = volM, molesM, minPc
        ...
        # else:
            ...
            # if vol1==0.0:
                # pc2 = 1e6
                # vol2, moles2 = returnClusterVolMoles(k, mem, pc2, halfAng, m_exists, 
                    # m_inited, m_initOrMaxPcHist, m_initOrMinApexDistHist, advPc, recPc, 
                    # m_initedApexDist, is_oil_inj, totElements, sigma, thetaAdvAng, thetaRecAng, 
                    # nCorners_arr, areaSPhase, _cornArea, _centerArea, conAng_cur, apexDist_cur, 
                    # volarray, satList, clusterID, imposedP, molecular_length, _delta, RT, 
                    # accurate, first_time)

            # else:
                # pc2 = moles_k*RT/vol1 - imposedP
                # vol2, moles2 = returnClusterVolMoles(k, mem, pc2, halfAng, m_exists, 
                    # m_inited, m_initOrMaxPcHist, m_initOrMinApexDistHist, advPc, recPc, 
                    # m_initedApexDist, is_oil_inj, totElements, sigma, thetaAdvAng, thetaRecAng, 
                    # nCorners_arr, areaSPhase, _cornArea, _centerArea, conAng_cur, apexDist_cur, 
        ...
        f1 = moles1 - moles_k
        f2 = moles2 - moles_k
        if (f1 * f2 > 0.0) or (abs(pc2 - pc1) <= pc_tol):
            clusterVol[k] = volC
            new_conc = H * (imposedP + currPc)
            for j in range(mem.size):
                m = mem[j]
...
@njit(void(
    ...
    float64, float32[:], float32[:], int32[:], float32[:], float32[:], float32[:],
    float32[:], float32[:], float64[:], float64[:], int32[:], float64[:], float32[:],
    float32[:], float32[:], float64[:], int32[:], int32[:], int32[:], int32[:], float64[:], 
    float64, float64, float64, float64, float64, float64, float64, float64, float64), cache=True)
def adjustClusterPcVolume_numba(cNetMolesAfterLastUpdate, halfAng, m_exists, m_inited, m_initOrMaxPcHist, 
    ...
    sigma, thetaAdvAng, thetaRecAng, nCorners_arr, areaSPhase, _cornArea, _centerArea, 
    conAng_cur, apexDist_cur, volarray, satList, clusterID, clusterVol, clusterPc, 
    pcShrink, pcMax, clusterMoles, members, mem_offsets, sizes, tempID, gasConc, 
    imposedP, molecular_length, _delta, H, RT, minPc, maxPc, moles_tol, pc_tol):
    ...
    for k in range(nClusters):
        #if sizes[k]:
        if cNetMolesAfterLastUpdate[k] > moles_tol:
            tempID[i] = k 
            i += 1
        elif cNetMolesAfterLastUpdate[k] < -moles_tol:
            j -= 1       
            tempID[j] = k
