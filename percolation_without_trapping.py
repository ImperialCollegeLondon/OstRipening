...
class PDrainage:    
    def __init__(self, obj, writeData=False, writeTrappedData=True):
        ...
        tPhaseD.__fileName__ = self.__fileName__
        tPhaseD.__writeTrappedData__ = self.__writeTrappedData__
...
def popUpdateOilInj(self):
    ...
    quickClustering(self, k, arr1, capPres)
...
class SecDrainage:
    def __init__(self, obj, writeData=False, writeTrappedData=True):
        ...
        obj.writeTrappedData = writeTrappedData
        tPhaseD.popUpdateOilInj = popUpdateOilInj
        tPhaseD.__fileName__ = PDrainage.__fileName__
        tPhaseD.__writeTrappedData__ = PDrainage.__writeTrappedData__
    ...
    def initialize(self):
        ...
        self.ElemToFill = SortedList(key=lambda i: tPhaseD.LookupList(self, i))
        self.NinElemList[:] = True
        initializeToFill(self)  
...
def initializeToFill(self):
    ...
    tPhaseD.__update_PcD_ToFill__(self, ElemToFill)
...
class PImbibition:
    def __init__(self, obj, writeData=False, writeTrappedData=True):
        obj.writeData = writeData
        obj.writeTrappedData = writeTrappedData
        tPhaseImb.popUpdateWaterInj = popUpdateWaterInj
        tPhaseImb.__PImbibition__ = __PImbibition__
        tPhaseImb.__fileName__ = self.__fileName__
        tPhaseImb.__writeTrappedData__ = self.__writeTrappedData__
...
def popUpdateWaterInj(self):
    ...
    do.check_Trapping_Clustering(
        self, neighb.copy(), self.hasNWFluid.copy(), 1, self.capPresMin,True)
    tPhaseImb.__computePc__(self, self.capPresMin, neighb, trapping=False)            
...
def __PImbibition__(self):
    ...
    while (self.PcTarget-1.0e-32 < self.capPresMin) & (
        ...
        try:
            while (self.invInsideBox < self.fillTarget) & (
                ...
                try:
                    ...
                    tPhaseImb.popUpdateWaterInj(self)
                except AssertionError:
                    self.filling = False
        ...
        tPhaseImb.__CondTPImbibition__(self)
...
class SecImbibition:
    ...
    def initialize(self):    
        self.fluid[[-1, 0]] = 0, 1  
        self.ElemToFill = SortedList(key=lambda i: tPhaseImb.LookupList(i, self.PcI, self.nPores))
        ...
        tPhaseImb.__computePc__(self, self.maxPc, self.elementLists, trapping=False)
