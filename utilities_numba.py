...
from OstRipening.numba_specs import *
import numpy as np

import pnflowPy.utilities as do
import OstRipening.cluster as doClust

#@njit(updateToDrainImbibe_spec, parallel=True, cache=True)
def updateToDrainImbibe_numba1(arrkeys, members, neighbours, toDrainArray, toImbibeArray, 
    ...
    def _func(i):
        ...
        #drain = neigh[np.argmin(PcI[neigh])]
...
def updateToDrainImbibe_single(k, mem, neigh, toDrainArray, toImbibeArray, 
    pcShrinkArray, pcMaxArray, PcI, PcD, totElements, tempMemArray, pc_min, pc_max):
...
def updateToDrainImbibe_numba(arrkeys, members, members_offset, size, neighbours, toDrainArray, toImbibeArray, 
    pcShrinkArray, pcMaxArray, PcI, PcD, Rarray, totElements, tempMemArray, parallel, pc_min, pc_max):
    ''' updates the toDrain and toImbibe arrays'''

    for k in arrkeys:
        updateToDrainImbibe_single(k, members[members_offset[k]:members_offset[k]+size[k]], 
            np.array(neighbours[k], dtype=np.int32), toDrainArray, toImbibeArray, pcShrinkArray, 
            pcMaxArray, PcI, PcD, totElements, tempMemArray, pc_min, pc_max)
...
def computeCornerArea_numba(arr, newPc, contactAng, cornA, m_halfAngles, m_cornExists, m_initOrMaxPcHist, 
    ...
    if arrS.any():
        conAngPS, apexDistPS = do.corner_apex_numba(
            arrS, m_halfAngles, newPc, contactAng, m_cornExists, m_initOrMaxPcHist, m_initOrMinApexDistHist, 
    ...
    if arrT.any():    
        conAngPT, apexDistPT = do.corner_apex_numba(
            arrT, m_halfAngles, newPc, contactAng, m_cornExists, m_initOrMaxPcHist, m_initOrMinApexDistHist, 
            m_advPc, m_recPc, m_initedApexDist, trappedW, trappedNW, clusterW_pc, clusterNW_pc, 
            clusterW_ID, clusterNW_ID, sigma, thetaAdvAng, thetaRecAng, _delta, overideTrapping, 
            MOLECULAR_LENGTH, 3)
...
def computeVolume_numba(keys, memkeys, memID, newPc, contactAng, cornA, m_halfAngles, m_cornExists, m_initOrMaxPcHist, 
    ...
    nElem = memID.size
    computeCornerArea_numba(memID, newPc[clusterNW_ID], contactAng, cornA, m_halfAngles, m_cornExists, m_initOrMaxPcHist, 
        m_initOrMinApexDistHist, m_advPc, m_recPc, m_initedApexDist, trappedW, trappedNW, clusterW_pc, 
        clusterNW_pc, clusterW_ID, clusterNW_ID, areaSPhase, maxCornerArea, sigma, thetaAdvAng, thetaRecAng, 
        _delta, isSquare, isTriangle, muw, MOLECULAR_LENGTH, overidetrapping)
...
def computeVolume_single_numba(memID, newPc, contactAng, cornA, m_halfAngles, m_cornExists, m_initOrMaxPcHist, 
    m_initOrMinApexDistHist, m_advPc, m_recPc, m_initedApexDist, trappedW, trappedNW, clusterW_pc, clusterNW_pc, 
    clusterW_ID, clusterNW_ID, areaSPhase, maxCornerArea, volarray, sigma, thetaAdvAng, thetaRecAng, PcD, 
    ...
    computeCornerArea_numba(memID_left, newPc, contactAng, cornA, m_halfAngles, m_cornExists, m_initOrMaxPcHist, 
        m_initOrMinApexDistHist, m_advPc, m_recPc, m_initedApexDist, trappedW, trappedNW, clusterW_pc, 
        clusterNW_pc, clusterW_ID, clusterNW_ID, areaSPhase, maxCornerArea, sigma, thetaAdvAng, thetaRecAng, 
        _delta, isSquare, isTriangle, muw, MOLECULAR_LENGTH, overidetrapping)
...
def returnVolMoles(keys, memkeys, memID, pc, contactAng, m_halfAngles, m_cornExists, m_initOrMaxPcHist, 
    ...
    _delta, isSquare, isTriangle, cornA, muw, MOLECULAR_LENGTH, imposedP, RT, updateSat):
    vol = computeVolume_numba(keys, memkeys, memID, pc, contactAng, cornA, m_halfAngles,
        ...
        satList, _delta, isSquare, isTriangle, muw, MOLECULAR_LENGTH, True, updateSat)
...
def findPcVolume(keys, memkeys, memID, targetMoles, contactAng, m_halfAngles, m_cornExists, m_initOrMaxPcHist, 
    m_initOrMinApexDistHist, m_advPc, m_recPc, m_initedApexDist, trappedW, trappedNW, clusterW_pc, clusterNW_pc, 
    ...
    vol1, moles1 = returnVolMoles(keys, memkeys, memID, pc1, contactAng, m_halfAngles, m_cornExists, 
    ...
    pc2[keys_notdone] = np.minimum(pcMax[keys_notdone]+1e-3, PcD.max())*(f_1[notdone_keys]<0)+(
        np.maximum(pcShrink[keys_notdone]-1e-3, 1e-3)*(f_1[notdone_keys]>0))
    vol2, moles2 = returnVolMoles(keys, memkeys, memID, pc2, contactAng, m_halfAngles, m_cornExists, 
        m_initOrMaxPcHist, m_initOrMinApexDistHist, m_advPc, m_recPc, m_initedApexDist, 
        trappedW, trappedNW, clusterW_pc, clusterNW_pc, clusterW_ID, clusterNW_ID, 
        areaSPhase, maxCornerArea, volarray, sigma, thetaAdvAng, thetaRecAng, PcD, satList, 
        tempClustVol, tempMemVol, _delta, isSquare, isTriangle, cornA, muw, MOLECULAR_LENGTH, 
    ...
    for _ in range(max_iter):
        ...
        vol2[notdone_keys], moles2[notdone_keys] = returnVolMoles(
            keys_notdone, memkeys, memID, pcNext, contactAng, m_halfAngles, m_cornExists, m_initOrMaxPcHist, 
